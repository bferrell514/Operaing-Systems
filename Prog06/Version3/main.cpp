...
void initializeApplication(void)
{
    ...
    lockGrid = new pthread_mutex_t[numRows * numCols];
    lockGrid2D = new pthread_mutex_t * [numRows];
    for (unsigned int i = 0; i < numRows * numCols; i++)
    {
    	pthread_mutex_init(&lockGrid[i], NULL);
    }
    lockGrid2D[0] = lockGrid;
    for (unsigned int i = 1; i < numRows; i++)
    	lockGrid2D[i] = lockGrid2D[i - 1] + numCols;
...
void lockMutexes(unsigned int row, unsigned int col)
{
	// Create index matrix for 3x3 cells
	unsigned int indices[9][2];
	for (int i = 0; i < 9; i++)
	{
		indices[i][0] = (row + numRows - (i / 3) + 1) % numRows;
		indices[i][1] = (col + numCols - (i % 3) + 1) % numCols;
	}
	// Sort the index matrix top-to-bottom, left-to-right
	for (int i = 0; i < 9; i++)
	{
		for (int j = i + 1; j < 9; j++)
		{
	...
	for (int i = 0; i < 9; i++)
	{
		pthread_mutex_lock(&lockGrid2D[indices[i][0]][indices[i][1]]);
	}
}

void unlockMutexes(unsigned int row, unsigned int col)
{
	// Create index matrix for 3x3 cells
	unsigned int indices[9][2];
	for (int i = 0; i < 9; i++)
	...
	for (int i = 0; i < 9; i++)
	{
		pthread_mutex_unlock(&lockGrid2D[indices[i][0]][indices[i][1]]);
	}
}

void* threadFunc(void* arg)
{
	// Inifinite loop until a stop signal from the main process arrives
	while (!stopThreads)
	{
		// Select a random cell
		unsigned int i = rand() % numRows;
		unsigned int j = rand() % numCols;
		// Lock 3x3 cells
		lockMutexes(i, j);
		// Update cell
		currentGrid2D[i][j] = cellNewState(i, j);
		//	In black and white mode, only alive/dead matters
		//	Dead is dead in any mode
		if (colorMode == 0 || currentGrid2D[i][j] == 0)
		{
			// Do nothing
		}
...
void cleanupAndquit(void)
{
	//  Wait until all calculation threads are finished
    for (unsigned int i = 0; i < numLiveThreads; i++)
    {
	...
	for (unsigned int i = 0; i < numRows * numCols; i++)
	{
		pthread_mutex_destroy(&lockGrid[i]);
	}
...
void myTimerFunc(int value)
{
    ...
    for (unsigned int i = 0; i < numRows; i++)
    {
    	...
    	{
    		pthread_mutex_lock(&lockGrid2D[i][j]);
    	}
    }
    ...
    for (unsigned int i = 0; i < numRows; i++)
    {
    	for (unsigned int j = 0; j < numCols; j++)
    	{
    		pthread_mutex_unlock(&lockGrid2D[i][j]);
    	}
    }
